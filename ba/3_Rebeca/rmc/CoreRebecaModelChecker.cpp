...
void CoreRebecaModelChecker::exportModelCheckingResult(string userSelectedPropertyName, string userSelectedPropertyType,
	...
	if (result != HEAP_OVERFLOW && result != SATISFIED && result != UNKNOWN_PROPERTY)
		printCounterExample();
    out  <<  "</counter-example-trace>"  <<  std::endl;
...
void CoreRebecaModelChecker::doDFSModelChecking(string userSelectedPropertyName, string userSelectedPropertyType) {
 ...
 claimTurn:
	...
	claims = getNextClaimState(stack[effectiveDepth].state->claim, propertySelector);	// we may use recently executed rebec
	...
	*/
	...
	for (std::list<short int>::iterator it = claims.begin(); it != claims.end(); ++it) {
 		...
 		if (result & NEW_STATE) {	// bitwise AND
			    ...
			    exportTransition(stack[depth].state, newState, sender, rebecs[r]->getName(), label, statespace);
		    #endif
...
void CoreRebecaModelChecker::printCounterExample() {
	...
	short int rebecId = stack[0].rebec;
	short int parent = 0;
	
    for (int cnt = 1; cnt <= depth; cnt++) {
        // non_det BFS state(not visited in DFS)
        if (stack[cnt].rebec == (byte)-1 && cnt != depth)
            continue;
		...
		exportTransition(stack[parent].state, stack[cnt].state, sender, rebecName, action, out);
		exportState(stack[cnt].state, out);
		rebecId = stack[cnt].rebec;
        parent = cnt;
	}
}
